...
C     AND MCS-7906671.
C.
C     V.Niess, 23-Nov-2009: I Modified the algorithm in order to allow a stop 
C     during function or gradient evaluation. On IV(1) = -1, the algorithm 
C     restarts with a function evaluation, on IV(1) = -2 the algorithms restarts 
C     with a gradient evaluation. This allows to split the minimisation over
C     several successive jobs when the objective function evaluation 
...
C
      ...
      IV1 = IV(1)
      IF (IV1 .EQ. -1) GO TO 25
      IF (IV1 .EQ. -2) GO TO 35
